...
category1 = ['natgas', 'natgas_ccs', 'natgas_fixed', 'natgas_ccs_fixed', 'dac', 'biopower']
...
def Update_info(info, category, results_tech_dic, results_case_dic, results_time_dic, input_case_dic, cost):
    if category in category1:
        ...
        info[category+'_var'].append(np.mean(cost_var * results_time_dic[category+' dispatch']))
        info[category+'_tax'].append(np.mean(cost_co2 * results_time_dic[category+' dispatch'] * co2_price_n))
...
def Get_Table(default_case_name, **kwargs):
    ...
    for tech_idx in tech_name_list:
        if tech_idx in category1:
            initial_keys(info, [tech_idx+'_cap', tech_idx+'_tot', tech_idx+'_emis', tech_idx+'_fix', tech_idx+'_var', tech_idx+'_tax'])
    ...
    def get_individual_case_result(case_name):
        ...
        if 'cost' not in info.keys():
            info['cost'] = {}
            for tech_idx in range(len(input_tech_list)):
                current_tech_list = input_tech_list[tech_idx]
                tech_name = current_tech_list['tech_name']
                if 'fixed_cost' in current_tech_list.keys():
                    info['cost'][tech_name+'_fix'] = [current_tech_list['fixed_cost']]
        else:
            for tech_idx in range(len(input_tech_list)):
                current_tech_list = input_tech_list[tech_idx]
                tech_name = current_tech_list['tech_name']
                if 'fixed_cost' in current_tech_list.keys():
                    info['cost'][tech_name+'_fix'].append(current_tech_list['fixed_cost'])
