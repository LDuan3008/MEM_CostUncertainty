...
if flag_scenario == 0:    # All high
    ccs_fhc = cost_transient['CCS_fhc_H']
    solar_fhc = cost_transient['solar_fhc_H']
    wind_fhc = cost_transient['wind_fhc_H']
    offwind_fhc = cost_transient['offwind_fhc_H']
    storage_fhc = cost_transient['storage_fhc_H']
    geothermal_fhc = cost_transient['geothermal_fhc_H']
if flag_scenario == 1:    # Geo low
    ccs_fhc = cost_transient['CCS_fhc_H']
    solar_fhc = cost_transient['solar_fhc_H']
    wind_fhc = cost_transient['wind_fhc_H']
    offwind_fhc = cost_transient['offwind_fhc_H']
    storage_fhc = cost_transient['storage_fhc_H']
    geothermal_fhc = cost_transient['geothermal_fhc_L']
if flag_scenario == 2:   # All low
    ccs_fhc = cost_transient['CCS_fhc_L']
    solar_fhc = cost_transient['solar_fhc_L']
    wind_fhc = cost_transient['wind_fhc_L']
    offwind_fhc = cost_transient['offwind_fhc_L']
    storage_fhc = cost_transient['storage_fhc_L']
    geothermal_fhc = cost_transient['geothermal_fhc_L']
...
for year_idx in np.arange(2019, 2051, 1):
    ...
    if current_idx == 0:
        ...
        print ('check: ', current_idx)
        search_list = case_dic['output_path'] + '/' + f'time_transient_sub{str(flag_scenario)}_year{str(year_idx-1)}'
    ...
    case_dic['case_name'] = f'time_transient_sub{str(flag_scenario)}_year{str(year_idx)}'
